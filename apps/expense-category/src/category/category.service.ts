...
export class CategoryService {
  ...
  createCategoryFromRabbitMQ(
    categoryFromRabbitMQDto: CategoryFromRabbitMQDto,
    ...
    return this.categoryReposetory.save(
      this.categoryReposetory.create({ name: activity, type, description }),
    );
  }
  ...
  createCategory(
    ...
    return this.categoryReposetory.save(
      this.categoryReposetory.create({ name, type, description }),
    );
  }
  ...
  async updateCategory(
    ...
    const categoryExist = await this.categoryReposetory.findOne({
      where: { id },
    });
    const { name, type, description } = updateCategoryDto;

    if (!categoryExist) throw new Error('Category didnt exist!');
