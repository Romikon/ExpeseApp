...
export class TransactionService {
  ...
  async createTransaction(
    ...
    await this.cloudAMQP.sendMessage({
      type: type,
      sum: sum,
      activity: activity,
    });

    return this.transactionReposetory.save(
      this.transactionReposetory.create({
        budgetid,
        categoryid,
        type,
        sum,
  ...
  async updateTransaction(
    ...
    const transactionExist = await this.transactionReposetory.findOne({
      where: { id },
    });
    const { budgetid, categoryid, type, sum, activity } = updateTransactionDto;

    if (!transactionExist) throw new Error('Transaction didnt exist!');
